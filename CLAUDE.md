...
> Full phase-by-phase detail: [docs/build-history.md](docs/build-history.md)

### Completed Phases (one-line each)
- **Phase 1** — Foundation: shared auth/RBAC, config_service, audit, notifications, Kafka, OTel, CBS connectors (Finacle/BaNCS/FlexCube)
- **Phase 2** — CTS Core: vaults, NGCH adapter, all Temporal workflows (inward + outward), CTS activities, frontend ops workstation
- **Phase 3** — Observability: OTel, Langfuse, Grafana dashboards, PrometheusRule CRDs, SHAP panel
- **Phase 4** — EJ Module: all workflows (normalise/dispute/health), edge Go binary, ingestion gateway, frontend
- **Phase 5** — Hardening: AI cascade L1/L2, delta vault sync + Bloom filter, HA/DR Helm values, Chaos Mesh, pilot bank values, live demo pipeline, MCP connection config API+UI
- **Phase 6** — Multi-Scenario CTS: scanner mapper, EEH/IEH, mismatch queue, drawee reorder, SMB portal, Agency CC, SMB CBS push, E2E harness (8 sub-phases, COMPLETE)
- **Phase 7** — Pluggable Auth Connectors: SAML/LDAP/local, AuthConnectorFactory, local_auth_accounts migration
- **Phase 8** — Pre-Live Smoke Test Suite: entity-scoped backend + frontend
- **Phase 9** — Security Remediation: ASTRA-01 (9/10 routers closed), ASTRA-02 (Temporal wiring), ASTRA-03 (async env gate) — all CLOSED; ej.py + DI gaps still open
- **Phase 10** — Error→Incident Management: incident schema + signal emission (Phases 1+2 of 5); Phase 3 needs live Grafana OnCall
- **Phase 11** — Audit/Notification Gap Closure: AuditStreamConsumer, audit_service, ImmudbWriter adapter, local auth connector
- **Phase 12** — TOTP/MFA + MSV + hardcoded-value audit: full MFA pipeline, Kafka topics registry

### Immediate Next (priority order)
1. **ASTRA-01 on ej.py** — same test-token backdoor fix proven on 9 other routers; deliberately deferred
2. **Activity/workflow decorator + DI gaps** — `@activity.defn`/`@workflow.defn` missing on 8 activities + 3 workflows; worker.py cannot build a real `Worker()` today
3. **Incident Management Phase 3** — Grafana OnCall wiring + Layer 3 escalation-chain config UI (needs real infra)
4. **SAML/LDAP notification recipients** — SAML: needs login-time cache or SCIM (no "list all users" by protocol); LDAP: buildable but not started
5. **Wire AuthConnectorFactory into real login flow** — dev_auth_server.py + main.py bypass the connector system entirely; SAML/LDAP hooks still raise NotImplementedError
6. **NPCI API Modernisation Phase A** — REST transport + `shared/ngch_auth/`. Trigger: NPCI concept note acceptance. Full task list: [docs/npci-readiness-plan.md](docs/npci-readiness-plan.md)
7. **Pilot bank deployment** — smoke-test saraswat-coop Helm values against real K8s cluster
8. **Security hardening** — OWASP ZAP + manual pen test prep

---

## 17. NPCI API Modernisation — ASTRA Readiness Plan

> Full plan with all task items: [docs/npci-readiness-plan.md](docs/npci-readiness-plan.md)

- **Trigger:** NPCI accepts concept note in `docs/NPCI-CTS-Modernisation-ConceptNote.html` — not started yet
- **Phases:** A (REST API, 6 months) → B (Webhook push, 12 months) → C (MCP client, 24 months)
- **Bottom line:** ~70% of internal plumbing ready. Gap is entirely in NPCI-facing transport layer: `ngch_adapter` (REST client replacing SFTP), `shared/ngch_auth/` (3-layer: mTLS + API key + HMAC-SHA256), webhook receiver (Phase B). No changes needed to AI activities, vault, Temporal structure, CBS connectors, EJ module, frontend, or audit trail.

---

## 18. Gemini Technical Evaluation — Architecture Hardening (July 2026)

> Full evaluation with all decisions and config keys: [docs/gemini-evaluation.md](docs/gemini-evaluation.md)

**Source:** Google Gemini 1.5 Pro, full codebase (1.21 MB, 97 files) — "Generation 3 clearing platform". All 5 fixes shipped.

| Fix | What | Where |
|---|---|---|
| A | AI cascade: 7B (L1 fast) → 72B (L2 forensic), ~90% clear at L1 | `shared/ai/model_cascade.py`, `alteration.py`, `ocr.py` |
| B | Delta vault sync (15-min) + Bloom filter for canceled leaves | `DeltaVaultSyncWorkflow`, `cts.vault.delta.{bank_id}` |
| C | HA/DR Helm values: YugabyteDB RF=3, Kafka min.insync=2, Temporal warm-DR, Redis active-passive | `infra/helm/astra-platform/values.yaml` ha section |
| D | EJ integrity check (9th activity in EJNormalisationWorkflow) + reconciliation orphan scanner | `verify_canonical_integrity.py` |
| E | Notification debouncer: 60s window, P0 bypass, batch summary alert | `shared/notifications/debouncer.py` |

---

*Last updated: July 2026 | Maintained by Claude Code session*
