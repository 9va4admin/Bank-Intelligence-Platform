...
.DESCRIPTION
    Runs all steps in order with live progress:
      ...
      Step 6  Open API + Worker + Frontend in new windows
...
# Colours
function Write-Step { param($n,$t) Write-Host "" ; Write-Host "[Step $n] $t" -ForegroundColor Cyan }
...
Write-Info "First run pulls images from Docker Hub (~2-5 GB). Subsequent runs are instant."
Write-Host ""
...
foreach ($container in $services.Keys) {
    ...
    while ($elapsed -lt $timeout) {
        ...
        Write-Host "       Waiting for $label ... ($elapsed s)   `r" -ForegroundColor DarkGray -NoNewline
        Start-Sleep 5
        $elapsed += 5
...
function Start-ServiceWindow {
    ...
    $fullCmd = "`$host.UI.RawUI.WindowTitle = '$Title'; Set-Location '$RepoRoot'; . '$envPs1Abs'; Write-Host '  $Title' -ForegroundColor Cyan; Write-Host ''; $Command"
    Start-Process powershell -ArgumentList "-NoExit", "-Command", $fullCmd
}

Write-Info "Opening API window    (uvicorn :8010) ..."
Start-ServiceWindow -Title "ASTRA - API :8010" -Command "python -m uvicorn apps.api.main:app --host 0.0.0.0 --port 8010 --reload --reload-dir apps/api --reload-dir shared --reload-dir modules/cts --log-level info"
