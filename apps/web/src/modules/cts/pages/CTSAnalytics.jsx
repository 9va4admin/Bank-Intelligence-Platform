  ...
  return (
    <AppShell>
      <div className="flex-1 overflow-y-auto bg-navy-950 px-6 py-5">
        <h1 className="text-lg font-semibold text-white mb-5">Analytics</h1>

        {/* KPI strip */}
        <div className="grid grid-cols-5 gap-3 mb-6">
          {[
            { label: 'Today Total',    value: today.total,          color: 'text-white' },
            { label: 'STP Rate',       value: `${stpRate}%`,        color: 'text-emerald-400' },
            { label: 'Avg Agent Time', value: `${today.avg_ms}ms`,  color: 'text-gold-400' },
            { label: 'Human Reviews',  value: today.human,          color: 'text-amber-400' },
            { label: 'IET Breaches',   value: '0',                  color: 'text-emerald-400' },
          ].map(k => (
            <div key={k.label} className="bg-navy-900 border border-white/8 rounded-xl px-4 py-3">
              <div className="text-[10px] text-slate-600 mb-1">{k.label}</div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="bg-navy-900 border border-white/8 rounded-xl p-4">
            <div className="text-sm font-medium text-white mb-4">Daily Volume (7 days)</div>
                ...
                <div key={d.date} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full flex flex-col justify-end" style={{ height: '100px' }}>
                    <div
                      ...
                      style={{ height: `${(d.stp_confirm / maxTotal) * 100}px` }}
                    />
                    <div
                      className="w-full bg-red-500/50"
                      style={{ height: `${(d.stp_return / maxTotal) * 100}px` }}
                    />
                    <div
                      className="w-full bg-amber-500/50 rounded-b"
                      style={{ height: `${(d.human / maxTotal) * 100}px` }}
                    />
                  </div>
                  <span className="text-[9px] text-slate-600">{d.date.replace('Jun ', '')}</span>
                </div>
              ))}
            </div>
          ...
          <div className="bg-navy-900 border border-white/8 rounded-xl p-4">
            <div className="text-sm font-medium text-white mb-4">Fraud Score Distribution (today)</div>
            <div className="space-y-2">
              {FRAUD_DIST.map(d => (
                <div key={d.range} className="flex items-center gap-3">
                  <span className="text-[10px] text-slate-500 w-16 shrink-0">{d.range}</span>
                  <div className="flex-1 bg-white/5 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${d.range.startsWith('70') || d.range.startsWith('90') ? 'bg-red-500' : 'bg-gold-400/60'}`}
        ...
        {/* Return reasons */}
        <div className="bg-navy-900 border border-white/8 rounded-xl p-4">
          <div className="text-sm font-medium text-white mb-4">STP Return Reasons (7-day)</div>
          <div className="space-y-2">
            {RETURN_REASONS.map(d => (
              <div key={d.reason} className="flex items-center gap-3">
                <span className="text-[10px] text-slate-500 w-32 shrink-0">{d.reason.replace(/_/g, ' ')}</span>
                <div className="flex-1 bg-white/5 rounded-full h-2">
