export default function BatchStats({ stats }) {
  ...
  const tiles = [
    { label: 'Total Inward', value: stats.total_inward, color: 'text-white' },
  ...
  return (
    <div className="border-b border-white/8 bg-navy-900/60 backdrop-blur px-6 py-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="text-xs font-semibold text-white">Mumbai Clearing · {stats.date}</div>
      ...
      <div className="grid grid-cols-6 gap-3">
        {tiles.map((t) => (
          <div key={t.label} className="text-center">
            ...
            <div className="text-[10px] text-slate-600 mt-0.5">{t.label}</div>
          </div>
        ))}
      </div>
